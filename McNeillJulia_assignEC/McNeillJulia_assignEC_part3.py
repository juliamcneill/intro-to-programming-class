...
#set turtle window size and pen color
turtle.setup(500, 500)
turtle.pencolor("black")

#count number of commands contained in list
command_range = int(len(drawing) / 3)

#for each commmand, extract a small 3 index list
for i in range(0,command_range):
    command = drawing[(0 + 3*i):(3 + 3*i)]
    #direct pen based on whether letter is in the first index, and
    #which coordinates are in the next two indices, of each list
    if command[0] == 'S':
        turtle.penup()
        turtle.goto(command[1], command[2])
    elif command[0] == 'P':
        turtle.pendown()
        turtle.goto(command[1], command[2])
    elif command[0] == 'E':
